...
namespace Segmentation {
	...
	double Guass(double x) {
		...
		return (1.0 / (sqrt(2 * PI)))*exp(exponent);  //高斯密度分布函数
	}
					...
					for (unsigned long l = ltr; l < rbr; l++)
					{
						for (unsigned long m = ltc; m < rbc; m++)
						{
							...
							if (distant_sq <= sr*sr)
							{
								...
								double weight = Guass(color_distance / tr);   //做出权重
	...
	void meanshift_segmentation(const Image & src, Image & dst, const int minarea = 20, const double sr = 6, const double tr = 8) {
		...
		unsigned long i;
		for ( i = 0; i < filt[0].size(); i++) { temprow.push_back(0); }
		printf("for ok1!\n");
		for ( i = 0; i < filt.size(); i++) { Label.push_back(temprow); }
		printf("for ok!\n");
